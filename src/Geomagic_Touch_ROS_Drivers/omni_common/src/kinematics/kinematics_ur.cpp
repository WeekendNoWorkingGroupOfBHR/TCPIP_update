                                    ...
                                    double q2,double q3, double q4, double q5, double q6)
{
    ...
    S1_T_S2 <<
        cos(q2+1.575),   0,       sin(q2+1.575),    0,
              0,         1,             0,    S2_y_S1,
        -sin(q2+1.575),  0,       cos(q2+1.575),    0,
              0,   0,             0,    1;
    ...
    S2_T_E <<
        cos(q3),   0,       sin(q3),    0,
              0,   1,             0,    -S2_y_E,
        -sin(q3),  0,       cos(q3),    S2_z_E,
              0,   0,             0,    1;    

    Eigen::Matrix4d E_T_W1;
    E_T_W1 <<
        cos(q4+1.575),   0,       sin(q4+1.575),    0,
              0,   1,             0,    E_y_W1,
        -sin(q4+1.575),  0,       cos(q4+1.575),    E_z_W1,
              0,   0,             0,    1;  
                                ...
                                double q2,double q3, double q4, double q5, double q6)
{
    ...
    // a1 <<
    //     0,0,1;
    a1 = B_T_S1.topRows(3).leftCols(3) * a1;//topRows(n)即取从头数前n行，leftCols(n)即取从左数前n行
    // std::cout <<a1(2)<< std::endl;

    Eigen::Vector3d a2(0,1,0);
    a2 = B_T_S1.topRows(3).leftCols(3) * S1_T_S2.topRows(3).leftCols(3) * a2;


    Eigen::Vector3d a3(0,1,0);
...
// std::cout <<p1<< std::endl;
    ...
    temp1 = a1.cross(pT - p1);//cross即叉乘运算
...
void kinematics_ur::rotationVector(Eigen::Vector3d &rotationvector, Eigen::Matrix3d rot)
{
    ...
    else
    {
        ...
        rotationvector = (theta/(2*sin(theta))) * temp_vec;//旋转向量
        Eigen::Vector3d temp1;
        temp1 = rotationvector;
        //temp1.norm();
            // std::cout << theta<< std::endl;
        Eigen::AngleAxisd rotation_vector2;
        rotation_vector2.fromRotationMatrix(rot);//将旋转矩阵变为旋转向量
        double alpha;
...
{//Rodriques公式
    ...
    Rot = Imatrix + Imatrix4 + Imatrix3 * normVectorHat;//旋转矩阵
    targetMatrix <<
...
// void kinematics_ur::inverse_kinematics_smooth(double &q1, double &q2,double &q3, double &q4, double &q5, 
//                                 double &q6, double x, double y, double z, double rx, double ry, double rz)
// {
//     double lambda = 0.1;
//     double q1noNan=q1,q2noNan=q2,q3noNan=q3,q4noNan=q4,q5noNan=q5,q6noNan= q6;
...
//         error_p = error_p * 0.5;
//         error_p <<
//             error_p(0)+nowMatrix(0,3),error_p(1)+nowMatrix(1,3),error_p(2)+nowMatrix(2,3);
//         thelta = error_r.norm();
//         error_r.normalize();
//         error_r = 0.1* thelta * error_r;
//         // CaltargetMatrix(targetMatrix_step,error_p(0),error_p(1),error_p(2),
//         //                                         error_r(0),error(1),error(2));
//         inverse_kinematics(q1,q2,q3,q4,q5,q6,error_p(0),error_p(1),error_p(2),
//                                                     error_r(0),error(1),error(2));
//         kinematics(nowMatrix,q1,q2,q3,q4,q5,q6);
//         Error(error, targetMatrix, nowMatrix);
//         std::cout<< error.norm()<<std::endl;
//     }
//     inverse_kinematics(q1,q2,q3,q4,q5,q6,x,y,z,rx,ry,rz);
