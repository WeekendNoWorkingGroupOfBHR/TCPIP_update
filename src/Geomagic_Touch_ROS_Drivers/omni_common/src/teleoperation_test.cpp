...
geometry_msgs::Twist qika_msg;
geometry_msgs::Twist qik_msg;
geometry_msgs::Twist qqpa_msg;
geometry_msgs::Twist qqp_msg;
...
ros::Publisher qqp_publisher;
ros::Publisher qqpa_publisher;
ros::Publisher qika_publisher;
ros::Publisher qik_publisher;
...
void tele_callback(const geometry_msgs::Twist & msg)
{
  if(isFirstCall)
    ...
    {     
      ...
      q_data[0] = -(msg.linear.x - touch_last_x)*0.002 + rotation_matrix_origin_4(12);
      q_data[1] = -(msg.linear.y - touch_last_y)*0.002 + rotation_matrix_origin_4(13);
      q_data[2] = (msg.linear.z - touch_last_z)*0.002 + rotation_matrix_origin_4(14);
      ...
      rotation_matrix_d=rz.inverse()*rotation_matrix_d*rz;
      ...
      rotation_matrix_end=rotation_matrix_end;
      ...
      // rotation_vector_end=ry*rotation_vector_end;
      ...
      ROS_INFO("desire_pp:%.2f,%.2f,%.2f,%.2f,%.2f,%.2f",q_data[0],q_data[1],q_data[2],q_data[3],q_data[4],q_data[5]);
      ROS_INFO("m_dCurPos:%.2f,%.2f,%.2f,%.2f,%.2f,%.2f",m_dCurPos[0],m_dCurPos[1],m_dCurPos[2],m_dCurPos[3],m_dCurPos[4],m_dCurPos[5]);
      ...
      for(int i=0;i<6;i++){
        // qdot[i]=(q_ik[i]-q_last[i])/ttt;
        // qddot[i]=(qdot[i]-qdot_last[i])/ttt;

        qdot_qp[i]=(now_q.at(i)-last_q.at(i))/ttt;
        qddot_qp[i]=(qdot_qp[i]-qdot_last_qp[i])/ttt;
      }
   
      qik_msg.linear.x=q_data[0];
      qik_msg.linear.y=q_data[1];
      qik_msg.linear.z=q_data[2];
      qik_msg.angular.x=q_data[3];
      qik_msg.angular.y=q_data[4];
      qik_msg.angular.z=q_data[5];
      qik_publisher.publish(qik_msg);
      // //关节空间逆运动学解算的加速度
      // qika_msg.linear.x=qddot[0];
      // qika_msg.linear.y=qddot[1];
      // qika_msg.linear.z=qddot[2];
      // qika_msg.angular.x=qddot[3];
      // qika_msg.angular.y=qddot[4];
      // qika_msg.angular.z=qddot[5];
      // qika_publisher.publish(qika_msg);

       //关节空间qp解算的速度
      qqp_msg.linear.x=qdot_qp[0];
      qqp_msg.linear.y=qdot_qp[1];
      qqp_msg.linear.z=qdot_qp[2];
      qqp_msg.angular.x=qdot_qp[3];
      qqp_msg.angular.y=qdot_qp[4];
      qqp_msg.angular.z=qdot_qp[5];
      qqp_publisher.publish(qqp_msg);
      //关节空间qp解算的加速度
      qqpa_msg.linear.x=qddot_qp[0];
      qqpa_msg.linear.y=qddot_qp[1];
      qqpa_msg.linear.z=qddot_qp[2];
      qqpa_msg.angular.x=qddot_qp[3];
      qqpa_msg.angular.y=qddot_qp[4];
      qqpa_msg.angular.z=qddot_qp[5];
      qqpa_publisher.publish(qqpa_msg);
...
//机械臂系统设定的初始位姿
  ...
  qik_publisher = nh_p.advertise<geometry_msgs::Twist>("qik_v",100);
  qika_publisher = nh_p.advertise<geometry_msgs::Twist>("qik_a",100);
  qqp_publisher = nh_p.advertise<geometry_msgs::Twist>("qqp_v",100);
  qqpa_publisher = nh_p.advertise<geometry_msgs::Twist>("qqp_a",100);
